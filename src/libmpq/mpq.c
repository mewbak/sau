...
int32_t libmpq__archive_open(mpq_archive_s *mpq_archive, const char *mpq_filename) {
    ...
    /*printf("archive_offset: %x\n", archive_offset);
    printf("hash_entry_size %d\n", sizeof(mpq_hash_s));
#define PHDR(x) printf("%s: %x\n", #x, mpq_archive->mpq_header->x)
    PHDR(header_size);
    PHDR(archive_size);
    PHDR(version);
    PHDR(block_size);
    PHDR(hash_table_offset);
    PHDR(block_table_offset);
    PHDR(hash_table_count);
    PHDR(block_table_count);*/
...
#undef PHDR
#define PHDR(x) printf("%s:%x ", #x, mpq_archive->mpq_hash[i].x)
		PHDR(hash_a); PHDR(hash_b); PHDR(locale); PHDR(platform); PHDR(block_table_index); printf("\n");
		abort();
	}*/
