//Credits go to Lutz Sammer, who created Freecraft.
...
{X,0,"human/01h",                                      65, 4 _2},
{X,0,"orc/01o",                                        66, 4 _2},
{X,0,"human/02h",                                      67, 4 _2},
{X,0,"orc/02o",                                        68, 4 _2},
{X,0,"human/03h",                                      69, 4 _2},
{X,0,"orc/03o",                                        70, 4 _2},
{X,0,"human/04h",                                      71, 4 _2},
{X,0,"orc/04o",                                        72, 4 _2},
{X,0,"human/05h",                                      73, 4 _2},
{X,0,"orc/05o",                                        74, 4 _2},
{X,0,"human/06h",                                      75, 4 _2},
{X,0,"orc/06o",                                        76, 4 _2},
{X,0,"human/07h",                                      77, 4 _2},
{X,0,"orc/07o",                                        78, 4 _2},
{X,0,"human/08h",                                      79, 4 _2},
{X,0,"orc/08o",                                        80, 4 _2},
{X,0,"human/09h",                                      81, 4 _2},
{X,0,"orc/09o",                                        82, 4 _2},
{X,0,"human/10h",                                      83, 4 _2},
{X,0,"orc/10o",                                        84, 4 _2},
{X,0,"human/11h",                                      85, 4 _2},
{X,0,"orc/11o",                                        86, 4 _2},
{X,0,"human/12h",                                      87, 4 _2},
{X,0,"orc/12o",                                        88, 4 _2},
{X,0,"human/13h",                                      89, 4 _2},
{X,0,"orc/13o",                                        90, 4 _2},
{X,0,"human/14h",                                      91, 4 _2},
{X,0,"orc/14o",                                        92, 4 _2},
{X,2,"human-exp/x01h",                                 99, 4 _2},
{X,2,"orc-exp/x01o",                                   100, 4 _2},
{X,2,"human-exp/x02h",                                 101, 4 _2},
{X,2,"orc-exp/x02o",                                   102, 4 _2},
{X,2,"human-exp/x03h",                                 103, 4 _2},
{X,2,"orc-exp/x03o",                                   104, 4 _2},
{X,2,"human-exp/x04h",                                 105, 4 _2},
{X,2,"orc-exp/x04o",                                   106, 4 _2},
{X,2,"human-exp/x05h",                                 107, 4 _2},
{X,2,"orc-exp/x05o",                                   108, 4 _2},
{X,2,"human-exp/x06h",                                 109, 4 _2},
{X,2,"orc-exp/x06o",                                   110, 4 _2},
{X,2,"human-exp/x07h",                                 111, 4 _2},
{X,2,"orc-exp/x07o",                                   112, 4 _2},
{X,2,"human-exp/x08h",                                 113, 4 _2},
{X,2,"orc-exp/x08o",                                   114, 4 _2},
{X,2,"human-exp/x09h",                                 115, 4 _2},
{X,2,"orc-exp/x09o",                                   116, 4 _2},
{X,2,"human-exp/x10h",                                 117, 4 _2},
{X,2,"orc-exp/x10o",                                   118, 4 _2},
{X,2,"human-exp/x11h",                                 119, 4 _2},
{X,2,"orc-exp/x11o",                                   120, 4 _2},
{X,2,"human-exp/x12h",                                 121, 4 _2},
{X,2,"orc-exp/x12o",                                   122, 4 _2},
...
*/
static void CheckPath(const char* path)
{
	char* cp;
	char* s;
...
*/
static void ConvertToMac(char* filename)
{
...
*/
static int SavePNG(const char* name, unsigned char* image, int x, int y, int w,
...
*/
static int OpenArchive(const char* file, int type)
{
 long archsize;
	int f;
 ...
 archsize = stat_buf.st_size;
 //printf("Filesize %ld %ldk\n", 	(long)archsize, sarchsize / 1024);

	//
	//  Read in the archive
	//
	buf = (unsigned char *)malloc(archsize);
	if (!buf) {
		printf("Can't malloc %ld\n", (long)archsize);
		exit(-1);
	}
	if (read(f, buf, stat_buf.st_size) != archsize) {
...
//	printf("Magic\t%08X\t", i);
   ...
   DemoVersion = 1;
		...
		printf("Wrong magic %08x, expected 0x19 (retail) or 0x18 (demo)\n", i);
		exit(-1);
	}
...
//	printf("ID\t%d\n", i);
	...
	for (i = 0; i < entries; ++i) {
  int off = FetchLE32(cp);
		op[i] = off > 0 ? buf + off : 0;
  //printf("%04d: offset=%d/%d\n", i, off, (int)archsize);
	}
	op[i] = buf + stat_buf.st_size;

	ArchiveOffsets = op;
 ArchiveOffsetsCount = entries;
	ArchiveBuffer = buf;
 ArchiveBufferEnd = ArchiveBuffer+archsize;
...
*/
int DecodeGfxEntry(int index, unsigned char* start,
	unsigned char* image, int ix, int iy, int iadd)
{
 ...
 if (width<=0 || height<=0 || offset<=0 || offset>856380) return 0;

 //printf("%2d: x=%2d y=%2d w=%2d h=%2d off=%d\n",
...
int ConvertGroupedGfu(char *path, int pale, int gfue, int glist)
{
 ...
 if (!gfup) return 0;
   ...
   if (!palp) return 0;
	    ...
	    printf("  Extracting: %s/%s/%s.png\n", GRAPHICS_PATH, path, gg->Name);
...
void ConvertPud(char* file, int pude)
{
 ...
 if (!pudp) return 0;
	...
	printf("  Extracting: %s/%s.pud\n", PUD_PATH, file);
	sprintf(buf, "%s/%s/%s.pud", Dir, PUD_PATH, file);
	CheckPath(buf);
	writeFile(0, l, buf, pudp);
...
int ConvertImage(char* file, int pale, int imge, int nw, int nh)
{
 ...
 if (!imgp) return 0;
	...
	if (!image) {
		printf("Could not extract image: file=%s pale=%d imge=%d nw=%d nh=%d mac=%d\n",
...
int ConvertCursor(char* file, int pale, int cure)
{
 ...
 if (!image || !w || !h || w>100 || h > 100) return 0;
...
int ConvertWav(char* file, int wave)
{
	...
	FILE *f;
	size_t l;
 if (ArchiveOffsets[wave]>ArchiveBufferEnd) return 0;
...
int ConvertText(char* file, int txte, int ofs)
{
 ...
 if (!txtp) return 0;
	...
	printf("  Extracting: %s/%s.txt\n", TEXT_PATH, file);
	sprintf(buf, "%s/%s/%s.txt", Dir, TEXT_PATH, file);
	CheckPath(buf);
...
**  Convert text to my format.
*/
static int SetupNames(char* file __attribute__((unused)), int txte __attribute__((unused)))
{
	unsigned char* txtp;
	const unsigned short* mp;
	size_t l;
...
int CampaignsCreate(char* file __attribute__((unused)), int txte, int ofs)
{
	...
	for (levelno = 0; levelno < expansion / 2; ++levelno) {
		for (race = 0; race < 2; ++race) {
			unsigned char *str;
            char *Name = Todo[2 * levelno + 1 + race + 5].File;
			sprintf(buf, "%s/%s/%s_obj.txt", Dir, TEXT_PATH, Name);
			CheckPath(buf);
            printf("  Extracting: %s/%s_obj.txt\n", TEXT_PATH, Name);
			outlevel = fopen(buf, "wb");
			str = ConvertString(CampaignData[race][levelno][0], 0);
			sprintf(buf, "[TITLE]\n%s\n", str);
			fputs(buf, outlevel);
			free(str);
			for (noobjs = 1; noobjs < 10; ++noobjs) {
				if (CampaignData[race][levelno][noobjs] != NULL) {
					str = ConvertString(CampaignData[race][levelno][noobjs], 0);
					sprintf(buf, "[OBJECTIVE]\n%s\n", str);
					fputs(buf, outlevel);
					free(str);
				}
			}
...
char *FTWar1[] = {
  ...
  "43.xmi",
  "44.xmi",
  "orc/11 - Goldshire and Moonbrook",
  "orc/11 - Goldshire and Moonbrook",
  "human/06 - Nortshire Abbey",
  "human/06 - Nortshire Abbey",
  "orc/03 - Grand Hamlet",
  "orc/03 - Grand Hamlet",
  "51",
  ...
  "orc/10 - Darkshire",
  "orc/10 - Darkshire",
  "human/02 - Grand Hamlet",
  "human/02 - Grand Hamlet",
  "human/05 - Elwynn Forest",
  "human/05 - Elwynn Forest",
  "orc/12 - Stormwind Keep",
  "orc/12 - Stormwind Keep",
  "61",
  "62",
  "human/01 - Rebuilding",
  ...
  "human/03 - Kyross",
  "human/03 - Kyross",
  "human/09 - Swamps of Sorrow",
  "human/09 - Swamps of Sorrow",
  "human/10 - Temple of the Damned",
  "human/10 - Temple of the Damned",
  "human/11 - Rockard and Stonard",
  "human/11 - Rockard and Stonard",
  "human/12 - Blackrock Spire",
  "human/12 - Blackrock Spire",
  "orc/01 - Swamps of Sorrow",
  "orc/01 - Swamps of Sorrow",
  "orc/02 - Swamps of Sorrow Borderlands",
  "orc/02 - Swamps of Sorrow Borderlands",
  "orc/05 - Rideridge Mountains",
  "orc/05 - Rideridge Mountains",
  "orc/07 - Blackrock Spire",
  "orc/07 - Blackrock Spire",
  "orc/09 - Southern Elwynn Forest",
  "orc/09 - Southern Elwynn Forest",
  "89",
  "90",
  "91",
  ...
  "orc/04 - Dead Mines",
  "orc/04 - Dead Mines",
  "human/08 - Medivhs Tower",
  "human/08 - Medivhs Tower",
  "human/04 - Dead Mines",
  "human/04 - Dead Mines",
  "orc/08 - Northshire Abbey",
  "orc/08 - Northshire Abbey",
  "101",
  "102",
  "103",
  ...
  "214",
  "215",
  "ui/blizzard_logo.png",
  "main.pal",
  "ui/human/top_bar.png",
  "ui/orc/top_bar.png",
  "ui/human/right_bar.png",
  "ui/orc/right_bar.png",
  "ui/human/bottom_bar.png",
  "ui/orc/bottom_bar.png",
  "ui/human/minimap.png",
  "ui/orc/minimap.png",
  "ui/human/left_panel.png",
  "ui/orc/left_panel.png",
  "ui/human/minimap2.png",
  "ui/orc/minimap2.png",
  "230",
  "231",
  "232",
  "ui/human/panel152x136.png",
  "ui/orc/panel152x136.png",
  "ui/human/panel286x48.png",
  "ui/orc/panel286x48.png",
  "ui/button192x15.png",
  "ui/button192x15_click.png",
  "ui/button112x15.png",
  "ui/button112x15_click.png",
  "ui/button54x15_click.png",
  "ui/button54x15_click.png",
  "ui/title_bottom.png",
  "ui/button_left.png",
  "ui/button_left_click.png",
  "ui/button_right.png",
  "ui/button_right_click.png",
  "ui/square83x75.png",
  "ui/human/save.png",
  "ui/orc/save.png",
  "ui/bar1.png",
  ...
  "277",
  "278",
  "279 - units human footman",
  "280 - units orc grunt",
  "281 - units human peasant",
  "282 - units orc peon",
  "283 - units human catapult",
  "284 - units orc catapult",
  "285 - units human knight",
  "286 - units orc rider",
  "287 - units human archer",
  "288 - units orc spearman",
  "289 - units human conjurer",
  "290 - units orc warlock",
  "291 - units human cleric",
  "292 - units orc necrolyte",
  "293 - units human medivh",
  "294 - units human lothar",
  "295 - units human lothar_wounded",
  "296 - units orc griselda",
  "297 - units neutral ogre",
  "298 - units neutral spider",
  "299 - units neutral slime",
  "300 - units neutral fire_elemental",
  "301 - units neutral scorpion",
  "302 - units neutral brigand",
  "303 - units neutral skeleton1",
  "304 - units neutral skeleton2",
  "305 - units neutral demon",
  "306 - units neutral water_elemental",
  "307 - buildings human farm",
  "308 - buildings orc farm",
  "309 - buildings human barracks",
  "310 - buildings orc barracks",
  "311",
  "312",
  "313",
  "314",
  ...
  "324",
  "325",
  "326 - units neutral corpses",
  "327 - units human peasant_wood",
  "328 - units orc peon_wood",
  "329 - units human peasant_gold",
  "330 - units orc peon_gold",
  "331",
  "332",
  "333",
  "334",
  ...
  "ui/victory.png",
  "ui/defeat.png",
  "ui/human/warroom.png",
  "ui/orc/warroom.png",
  "human_warroom.pal",
  "orc_warroom.pal",
  "425",
  "426 - orc_warroom_anim1",
  "427 - orc_warroom_anim2",
  "428 - human_warroom_anim1",
  "429 - human_warroom_anim2",
  "430",
  "431",
  "text/432.txt",
  "text/433.txt",
  ...
  "text/454.txt",
  "text/455.txt",
  "ui/human/warlord1.png",
  "human_warlord.pal",
  "ui/orc/warlord1.png",
  "orc_warlord.pal",
  "460",
  "text/orc_outro1.txt",
  "text/human_outro1.txt",
  "text/orc_outro2.txt",
  "text/human_outro2.txt",
  "465",
  "466",
  "467",
  "468",
  "469",
  "ui/human/warlord2.png",
  "ui/orc/warlord2.png",
  "misc/blizzard.wav",
  "misc/open_gates.wav",
  "unit/construction.voc",
  "unit/explosion.voc",
  "unit/ballista_shot.voc",
  "unit/woodcut1.voc",
  "unit/woodcut2.voc",
  "unit/woodcut3.voc",
  "unit/woodcut4.voc",
  ...
  "unit/ring.voc",
  "unit/click.wav",
  "unit/placement_error.voc",
  "unit/sword1.voc",
  "unit/sword2.voc",
  "unit/sword3.voc",
  "unit/fist.voc",
  "unit/magic_impact.voc",
  "unit/magic_shot.voc",
  "unit/arrow_shot.voc",
  "unit/arrow_hit.voc",
  "unit/orc_danger1.voc",
  "unit/orc_danger2.voc",
  "unit/human_danger1.wav",
  "unit/human_danger2.wav",
  "unit/orc_dead.voc",
  "unit/human_dead.voc",
  "unit/orc_work_complete.voc",
  "unit/human_work_complete.wav",
  "unit/orc_help.voc",
  "unit/orc_help_building.wav",
  "unit/human_help.wav",
  "unit/human_help_building.wav",
  "unit/orc_ready.voc",
  "unit/human_ready.wav",
  "unit/orc_ack1.voc",
  "unit/orc_ack2.voc",
  "unit/orc_ack3.voc",
  "unit/orc_ack4.voc",
  ...
  "unit/human_sel4.wav",
  "unit/human_sel5.wav",
  "unit/orc_annoyed1.voc",
  "unit/orc_annoyed2.voc",
  "unit/orc_annoyed3.wav",
  "unit/human_annoyed1.wav",
  "unit/human_annoyed2.wav",
  "unit/human_annoyed3.wav",
  "unit/spell1.wav",
  "unit/spell2.wav",
  "unit/placement_succeed.wav",
  "unit/temple.wav",
  "unit/church.wav",
  "unit/kennels.wav",
  "unit/stables.wav",
  ...
  "unit/shot1.wav",
  "unit/shot2.wav",
  "briefing/human_outro1.wav",
  "briefing/human_outro2.wav",
  "briefing/orc_outro1.wav",
  "briefing/orc_outro2.wav",
  "intro/1.wav",
  "intro/2.wav",
  "intro/3.wav",
  "intro/4.wav",
  "intro/5.wav",
  "briefing/human01.wav",
  "briefing/human02.wav",
  "briefing/human03.wav",
  "briefing/human04.wav",
  "briefing/human05.wav",
  "briefing/human06.wav",
  "briefing/human07.wav",
  "briefing/human08.wav",
  "briefing/human09.wav",
  "briefing/human10.wav",
  "briefing/human11.wav",
  "briefing/human12.wav",
  "briefing/orc01.wav",
  "briefing/orc02.wav",
  "briefing/orc03.wav",
  "briefing/orc04.wav",
  ...
  "briefing/orc11.wav",
  "briefing/orc12.wav",
  "briefing/human_loss.wav",
  "briefing/orc_loss.wav",
  "briefing/orc_victory1.wav",
  "briefing/orc_victory2.wav",
  "briefing/orc_victory3.wav",
  "briefing/human_victory1.wav",
  "briefing/human_victory2.wav",
  "briefing/human_victory3.wav",
  0};


static void saveImage(char *Output, u1 *M, int L, u1 *Pal) {
  int I, J, K, C, X, Y;
  int W = *(u2*)M;
  int H = *(u2*)(M+2);
  u1 *Q, *Z;
  ...
  M += 4;
  times (Y, H) times (X,W) picPut(P,X,Y,M[Y*W+X]);
  pngSave(Output, P);
  picDel(P);
}
...
static void saveSprite(char *Output, u1 *M, int L, u1 *Pal) {
  char Tmp[1024];
  int I, J, K, C, X, Y;
  int NFrames = *(u2*)M;
  int W = *(M+2);
  ...
  //printf("    NFrames=%d W=%d H=%d\n", NFrames, W, H);
  times(I, NFrames) {
    P = S->Anis[0].Facs[0].Pics[I] = picNew(W, H, 8);
    P->SharedPalette = 1;
    free(P->P);
    P->P = S->Palette;
    //printf("    %03d:%06x X=%d Y=%d W=%d H=%d\n"
    //      ,I, F[I].Off, F[I].X, F[I].Y, F[I].W, F[I].H);
    Q = M+F[I].Off;
    times (Y, F[I].H) times (X, F[I].W) picPut(P,F[I].X+X,Y,Q[Y*F[I].W+X]);
  }
...
static int isSprite(u1 *M, int L) {
  int I, W, H, NFrames;
  frame *F = (frame*)(M+4);
  if (L < 16) return 0;
  ...
  if (W<2 || H<2 || !NFrames || NFrames > 255 || NFrames*sizeof(frame)+4>=L) return 0;

  times(I, NFrames) if (F[I].Off>>24) return 0;

  return 1;
}

static void decompileWar1(char *Output, char *Input) {
  char Tmp[1024];
  int I, J, K, NFiles, Dummy, Off, Len, Enc, L = fileSize(Input);
  u1 *Z, *M = readFile(0, L, Input);
  u4 *O = (u4*)(M+8);

  if (*(u4*)M != 0x18) {
    printf("  Not a warcraft archive.\n");
    abort();
  }
  ...
  times (I, NFiles) {
    unless (FTWar1[I]) break; // something got wrong.
    Off = O[I];

    if (Off == 0xFFFFFFFF) continue; // not in this version of game

    Enc = *(u4*)(M+Off)>>24;
    Len = *(u4*)(M+Off)&0xFFFFFF;

    if (Off > L+4 || Len == 0x202020) continue; // not in this version of game

    printf("  Extracting: %s\n", FTWar1[I]);
    //printf("    %08x:%06x:%02x\n", Off, Len, Enc);
	if (Enc == 0x00) Z = M+Off+4;
    else if (Enc == 0x20) Z = ExtractEntry(M+Off, &Len);
    else continue;

    sprintf(Tmp, "%s/%s", Output, FTWar1[I]);

    //!strncmp(FTWar1[I], "images", 6)
    if (117 <= I && I <= 188) { // unit placement and objectives
      sprintf(Tmp, "%s/maps/%s.dat", Output, FTWar1[I]);
    } else if (Len > 4 && *(u2*)Z * *(u2*)(Z+2) == Len-4) {
      int Pal = 217;
      if (216 <= I && I <= 242 || I==250 || I==407 || I == 409 || I == 410) Pal = 217;
      if (254 <= I && I <= 257) Pal = 255; // hotkeys
      if (237 <= I && I <= 248) Pal = 260;

      // human panels
      if (I == 218 || I == 220 || I == 222 || I == 224 || I==226
          || I==228 || I==233 || I==235 || I==249)
        Pal = 260;

      if (I==249 || 256 <= I && I <= 261 || 262 <= I && I <= 365 || I==470 || I==471)
        Pal = 260;

      if (I==411 || I==419 || I==420) Pal = 413; // human outcome
      if (I==412) Pal = 414; // orc outcome
      if (I==416) Pal = 414; // outcome pic

      if (I == 415) Pal = 416; // victory pic
      if (I == 417) Pal = 418; // defeat pic

      if (I == 421) Pal = 423; // human warroom
      if (I == 422) Pal = 424; // orc warroom
      if (I == 456) Pal = 457; // human warlord
      if (I == 458) Pal = 459; // orc warlord

      sprintf(Tmp, "%s/images/%s", Output, FTWar1[I]);
      saveImage(Tmp, Z, Len, ExtractEntry(M+O[Pal], &Dummy));
      continue;
    } else if (Len == 256*3) {
      sprintf(Tmp, "%s/palettes/%s", Output, FTWar1[I]);
    } else if (Len == 1024*8) { // tile size is 16x16
      if (I%2) sprintf(Tmp, "%s/maps/%s.map", Output, FTWar1[I]);
      else sprintf(Tmp, "%s/maps/%s.mask", Output, FTWar1[I]);
    } else if (Len > 16 && !memcmp(Z,"FORM", 4)) {
      sprintf(Tmp, "%s/music/%s", Output, FTWar1[I]);
    } else if (Len > 16 && !memcmp(Z,"RIFF", 4)) {
      sprintf(Tmp, "%s/sound/%s", Output, FTWar1[I]);
    } else if (Len > 8 && !memcmp(Z,"Creative", 8)) {
      sprintf(Tmp, "%s/sound/%s", Output, FTWar1[I]);
    } else if (isSprite(Z, L)) {
      int Pal = 217;
      if (I==426 || I==427) Pal = 424; // orc warroom
      if (I==428 || I==429) Pal = 423; // human warroom
      if (I==359 || I==360) Pal = 217;
      sprintf(Tmp, "%s/images/%s", Output, FTWar1[I]);
      saveSprite(Tmp, Z, Len, ExtractEntry(M+O[Pal], &Dummy));
      continue;
    } else if (432 <= I && I <= 455 || 461 <= I && I <= 464) { // text
      sprintf(Tmp, "%s/data/%s", Output, FTWar1[I]);
    } else if (366 <= I && I <= 469) { // ui data
      sprintf(Tmp, "%s/data/ui/%s.ui", Output, FTWar1[I]);
    } else {
      sprintf(Tmp, "%s/data/%s", Output, FTWar1[I]);
    }
...
// FIXME: use tileset's palette
static void warDecompile(char *Output, char *Input) {
	...
	downcase(Name);
	if (!strcmp(Name, "data")) { // warcraft1?
		decompileWar1(Output, Input);
		return;
	}
		...
		if (st.st_size != 2811086) {
			...
			switch (st.st_size) {
				...
				default:
					...
					printf("Defaulting to US original DOS CD\n");
					CDType |= CD_US; //CD_GERMAN;
					break;
			}
		} else {
...
//#endif
			...
			case G:
				...
				break;
			case U:
				if (!DemoVersion) ConvertGfu(Todo[u].File, Todo[u].Arg1, Todo[u].Arg2);
				break;
			case D:
				if (!DemoVersion) ConvertGroupedGfu(Todo[u].File, Todo[u].Arg1, Todo[u].Arg2,
					Todo[u].Arg3);
				break;
			case P:
				...
				break;
			case N:
				if (!DemoVersion) ConvertFont(Todo[u].File, 2, Todo[u].Arg1);
				break;
			case I:
				...
				break;
			case C:
				if (!DemoVersion) ConvertCursor(Todo[u].File, Todo[u].Arg1, Todo[u].Arg2);
				break;
			case W:
...
int warInit(format *F) {
  F->Type = FMT_ARCHIVE;
  F->Name = "war";
  F->Description = "Warcraft 1 and 2 archives";
  F->Decompile = warDecompile;
  return 1;
}
